...
try {
	// Constantes
	define('TARGET', 'images/sponsor/'); // Repertoire cible
	...
	if(!empty($_POST)) {
		...
		if ($methode == "create") {
			...
			echo $ville."<br>";
			$managerSponsor->ajouterSponsor($nom, $url, $adresse, $codePostal, $ville, $tel, $fax, $email, $_FILES['photo']['name'], $user->getLogin());

		} else {
			$id = $_POST['id'];
			$managerSponsor->majSponsor($id, $nom, $url, $adresse, $codePostal, $ville, $tel, $fax, $email, $_FILES['photo']['name'], $user->getLogin());
		}
	}
